function ball = makeCyl(Nx, Ny, Nz, cx, cy, cz, radius, half_height , plot_ball, binary)
...
if nargin < 9 || isempty(plot_ball)
    plot_ball = false;
end

% check for binary input
if nargin < 10 || isempty(binary)
    binary = false;
end
...
% define pixel map
[r_xy, r_z] = makePixelMap_cyl(Nx, Ny, Nz, 'Shift', [0, 0, 0]);

% create disc
ball(r_xy < radius & r_z < half_height ) = MAGNITUDE;
