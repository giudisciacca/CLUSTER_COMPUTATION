...
lname = '/home/gdisciac/k-wave/myscripts/scripts_victre/VICTRE_PARADIGM_';
saving_predir =  '/home/gdisciac/k-wave/myscripts/scripts_victre'
saving_dir  = [saving_predir,filesep,'US_VICTRE_PARADIGM'];
mkdir(saving_dir)
if ~exist('i0','var')
i0 = 1;
end
...
M = 0;
if exist([saving_dir,filesep,type,num2str(i0),'VICTRE.mat'],'file')
FileInfo = dir([saving_dir,filesep,type,num2str(i0),'VICTRE.mat']);
...
for i = i0:iend
    ...
    % simulate
    [out_us, scan_lines_raw, sound_speed_map, backmean,backstd,sca_reg] = simulateUS_victre(voidMass, [dimVox,dimVox,dimVox]*10^-3, [type,num2str(i)], structkwave);
    % select prior
    ...
    %USim(i,:,:) = out_us;
    close all
    save([saving_dir,filesep,type,num2str(i),'VICTRE.mat'],'out_us','scan_lines_raw','sound_speed_map','backmean','backstd','sca_reg');%,'scan_lines_raw')
    % append
end
end
