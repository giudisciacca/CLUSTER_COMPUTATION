function [out,scan_lines_raw,sound_speed_map,background_map_mean,background_map_std,scattering_region] = simulateUS_victre(prior3D,prior3D_dd,part_name, hete_back)
...
pml_x_size = 30;%20;                % [grid points]
pml_y_size = 10;%128;                % [grid points]
pml_z_size = 5;%1;                % [grid points]

% set total number of grid points not including the PML
Nx = (3^2)*(2^6) - 2 * pml_x_size; %(3^3)*(2^3) - 2 * pml_x_size;      % [grid points](3^2)*(2^6)
Ny = (3^1)*(2^6) - 2 * pml_y_size;      % [grid points]
Nz = (3^1)*(2^5) - 2 * pml_z_size;      % [grid points]
...
layer_mm = 5;
hete_back=cat(1,hete_back(layer_mm:-1:1,:,:),hete_back);
prior3D=cat(1,0*prior3D(layer_mm:-1:1,:,:),prior3D);
...
transducer.number_elements = 42;  	% total number of transducer elements
transducer.element_width = ceil(0.000198 / dy);       % width of each element [grid points]
transducer.element_length = ceil(0.004 / dz);  	% length of each element [grid points]
transducer.element_spacing = 0*ceil(0.000002/dy);  	% spacing (kerf  width) between the elements [grid points]
...
transducer.focus_distance = 5e-3+20e-3;              % focus distance [m]
transducer.elevation_focus_distance = 5e-3+21e-3;    % focus distance in the elevation plane [m]
...
background_map_std_max = 0.05;
background_map_std_min = 0.01;

background_map_std = background_map_std_min+...
    (background_map_std_max  - background_map_std_min)*to_grid(hete_back, [Nx_tot,Ny_tot,Nz_tot], [dx,dy,dz],[prior3D_dx,prior3D_dy, prior3D_dz]);

background_map_mean = ones(size(background_map_std));
background_map_mean_max = 1.01;
background_map_mean_min = 0.99;
for iu = unique(background_map_std)'
    background_map_mean(background_map_std==iu) = background_map_mean_min + ...
        (background_map_mean_max - background_map_mean_min)*rand;
end


background_map = background_map_mean + background_map_std .* randn([Nx_tot, Ny_tot, Nz_tot]);
background_map(background_map*c0<1400) = 1400/c0;
background_map(background_map*c0>1600) = 1600/c0;

% define a random distribution of scatterers for the highly scattering
% region
scattering_map = randn([Nx_tot, Ny_tot, Nz_tot]);
sigma_scatter = 0.0000000005;
scattering_c0 = c0 + delta_scatter +  (sigma_scatter*c0) * scattering_map;
bound_sigmau =1600;% (c0 + delta_scatter) * (1 + 9*sigma_scatter);
bound_sigmad =1400;% (c0 + delta_scatter) * (1 - 9*sigma_scatter);
...
scattering_region=round(to_grid(prior3D, [Nx_tot,Ny_tot,Nz_tot], [dx,dy,dz],[prior3D_dx,prior3D_dy, prior3D_dz]));

% shift to get 5 mm more
layer_mm = round((5e-3)/dx);
%scattering_region=circshift(scattering_region,[layer_mm, 0, 0]);
%scattering_rho0=cat(1,scattering_rho0(end-layer_mm,:,:),scattering_rho0);
% assign region
sound_speed_map(scattering_region == 1) =   scattering_c0(scattering_region >= 0.9) ;
density_map(scattering_region == 1) = scattering_rho0(scattering_region >= 0.9);

% heterogeneous attenuation
alpha_coeff_map = 1.2*scattering_region+0.4*(~scattering_region);

%medium.alpha_coeff(scattering_region == 0) = 1.2;% [dB/(MHz^y cm)]
medium.alpha_coeff = alpha_coeff_map;
medium.alpha_power = 1.1;
medium.BonA = 5;
medium.sound_speed_ref = max(sound_speed_map(:));
% =========================================================================
% RUN THE SIMULATION
% =========================================================================
...
%dataPath = ['/scratch0/NOT_BACKED_UP/gdisciac/VICTRE/myresults/',part_name];
input_args = {...
    'PMLInside', false, 'PMLSize', [pml_x_size, pml_y_size, pml_z_size], 'PMLAlpha', [10,10,30]...
