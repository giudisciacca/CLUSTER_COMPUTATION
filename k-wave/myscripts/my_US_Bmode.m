...
Nx = (3^3)*(2^3) - 2 * pml_x_size;      % [grid points]
Ny = (3^2)*(2^4)  - 2 * pml_y_size;      % [grid points]
...
transducer.elevation_focus_distance = 21e-3;    % focus distance in the elevation plane [m]
...
number_scan_lines = 140;% - 2 * number_active_elements;
Nx_tot = Nx;
...
background_map = background_map_mean + 0.1 * background_map_std * randn([Nx_tot, Ny_tot, Nz_tot]);

% define a random distribution of scatterers for the highly scattering
% region
scattering_map = randn([Nx_tot, Ny_tot, Nz_tot]);
scattering_c0 = c0 + delta_scatter + 75 * scattering_map;
scattering_c0(scattering_c0 > 1600) = 1600; %[m/s]
scattering_c0(scattering_c0 < 1400) = 1400;
scattering_rho0 = scattering_c0 / 1.54;  % keep impedance constant
...
scattering_region = makeBall(Nx_tot, Ny_tot, Nz_tot, round(x_pos/dx), round(Ny_tot/2), round(z_pos/dz), round(radius/dx));
