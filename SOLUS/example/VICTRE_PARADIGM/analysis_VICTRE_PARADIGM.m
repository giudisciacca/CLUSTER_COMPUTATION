...
for isim_back = numel(Con):-1:1
    ...
    loadname = ['Test_Standard_PARADIGM2__type',type,'_tau5e-03_mu0MeanFittedSample',Con{isim},'_REC.mat'];
...
figure(1), scatter([Ground(:).muain]',[Qset(:).ValIn]')
%%
Ct =[];
Cr = [];
At = [];
...
for i = 1:1%numel(RECset)
    ...
    fh.NumberTitle = 'off';fh.Name = ['Truth Absorption Con', Con{i}];
...
close all
tmp = reshape([inAr';inSr'],[16,2*size(inArV,1)/16])';
%tmp1 = tmp;
tmp =tmp(:,1:end);
[coeff,score,latent] = pca(tmp);%,'VariableWeights','variance');
Xcentered = score*coeff';
figure
 ...
 a = [Ground(:).muain]';
 b = [Qset(:).ValIn]';
 plot(0:0.001:0.04,0:0.001:0.04),hold on
 for i = 1:21
     i
     scatter(a(1+(i-1)*8:(i)*8),b(1+(i-1)*8:(i)*8)),xlim([-0.02,0.04]),ylim([-0.02,0.04])
     hold on,
     pause
 end
 %plot(0:0.001:0.04,0:0.001:0.04)
 
  figure;
 a = [Ground(:).muspin]';
 b = [Qset(:).ValInSca]';
 scatter(a(1:1:end),b(1:1:end))%,xlim([0,0.04]),ylim([0,0.04])
 hold on, plot(0:0.1:2,0:0.1:2)
% truth
