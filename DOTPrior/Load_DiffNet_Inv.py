...
def extract_images(filename,imageName):
  ...
  data = data.transpose(numpy.flip(dims, 0))
  if data.ndim == 3:
    return data

  if data.ndim < 5 :
    data = numpy.expand_dims(data, data.ndim)
  print(data.shape)
  return data
...
class DataSet(object):
  ...
  def __init__(self, indata, list_of_arrays):
    ...
    for i in range(0, len(list_of_arrays)-1):
      ...
      #cmd_assert = "print(%s.shape[-1]); print(%s.shape[-1])" % (list_of_arrays[i], list_of_arrays[i + 1])
      exec(cmd_assert)
...
def read_data_sets(FileNameTrain,FileNameTest, string_array):
  ...
  data_sets = DataSets()
  #string_array = ['data', 'priorslice', 'prior', 'recona','recons', 'truea', 'trues','s','U','V', 'US'];
  string_array = ['priorslice', 'prior', 'recona', 'recons', 'truea', 'trues', 'US'];
  if 'USonly' in sys.argv[6]:
    string_array = ['priorslice', 'prior', 'recona', 'recons', 'truea', 'trues'];
  ...
  if FileNameTrain != 'absent':
    data_sets.train = DataSet(train, string_array)#[ ('train_').__add__(string_array[i]) for i in range(len(string_array))] )
  data_sets.test = DataSet(test,  string_array)#[ ('test_').__add__(string_array[i]) for i in range(len(string_array)) ] )
