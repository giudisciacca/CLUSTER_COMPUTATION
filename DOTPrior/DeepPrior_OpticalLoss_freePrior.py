    ...
    for i in range(0,np.shape(U)[0]):
        ...
        if global_behav == 'fromSingle':
            ...
            for i_prior in range(0,4):
                L[i_prior] = ssp_getLaplacian(prior_tot[:,:,:,:, (6*i_prior): (6*(i_prior+1))], str_behaviour='fromDiags');
...
def main(filePath, fileOutName, dataSetTrain, dataSetTest, tRand, MatOutName):
    ...
    with tf.name_scope('Net'):
        ...
        for i_rec in range(0,1):
            recon_prior = (UNET(muarecon0, musrecon0, tf.nn.sigmoid(prior), outchan=6*4)+ bb)
        ...
        Jp = assembleJ(J0, Lprior,Lprior, tf.zeros_like(Lprior),tf.zeros_like(Lprior))
        ...
        '''
        alpha = bias_variable([1])
        out = tf.py_function(func=ssp_solve,inp=[U,s,V,data,tf.nn.relu(recon_prior), alpha],Tout = tf.float32)
        out = tf.transpose(tf.reshape(out,[bSize,2, Nz, Ny, Nx, 1]), [0,4,3,2,1, 5]);
        out_mua = 1+out[:,:,:,:,0,:];
        out_mus = tf.div(1.0,3.0*(out[:,:,:,:,1,:]+1));

    with tf.name_scope('optimizer'):
        #rel_loss_p = tf.norm(tf.subtract(tf.nn.relu(recon_prior), prior3D)) / tf.norm(prior3D)
        ...
        rel_loss = rel_loss_a + 0.2*rel_loss_s;
        learningRate = tf.constant(1e-7)
    ...
    for i in range(maxIter):
        ...
        _, loss_result_train, rela, rels = sess.run([train_step, rel_loss, rel_loss_a, rel_loss_s], feed_dict=feed_train)
        Ltraintot.append(loss_result_train)
        Ltraina.append(rela)
        Ltrains.append(rels)
        ...
        if i % 50 == 0: #write test
            ...
            for i_test in range(0,len(dataDbar)):
                ...
                rel_loss_res,rel_loss_res_a,rel_loss_res_s= sess.run([ rel_loss,rel_loss_a,rel_loss_s],
                                                                            feed_dict=feed_test)

                Ltesta.append(rel_loss_res_a)
                Ltests.append(rel_loss_res_s)
                Ltesttot.append(rel_loss_res)
                print('iter={}, rel.loss={}, rel.lossa.test={}, rel.losss.train={}'.format(i,rel_loss_rel,rel_loss_res_a,rel_loss_res_s ))
    ...
    for i_dataset in range(0, len(dataDbar)):
        ...
        for i_final in range(0, np.shape(dataDbar[i_dataset].test.diffused)[0] - bSize + 1):
            tBeg = i_final
            tEnd = i_final + bSize;
            feed_test = {muarecon0: dataDbar[i_test].test.recona[tBeg:tEnd],
                         musrecon0: dataDbar[i_test].test.recons[tBeg:tEnd],
