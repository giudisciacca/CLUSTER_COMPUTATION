...
#        prcmax=100 - 5*tf.nn.sigmoid(bias_variable([1]))[0];
        prcmin= tf.constant(np.float32(sys.argv[7]));
        prcmax= 100 - prcmin ;
        
    elif type=='s':
        prcmin=tf.constant(np.float32(sys.argv[8]));
        prcmax= 100 - prcmin;
...
logNameCommon = folder_tensorboard +'prcSCA'+sys.argv[8] +'prcABS'+sys.argv[7] +'3UNET__';
