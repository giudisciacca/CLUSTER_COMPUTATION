...
def UNET(inValue0, inValue1, inValue2, outchan=1):
    ...
    wout2 = weight_variable([3, 3, 3, 3 * int(int(wchan) / 2), int(int(wchan) / 2)],
                            'zw10102' + str(randint(0, 100000)));
    l2_d = conv3d(tf.concat([b0, b1, b2], axis=4), wout2);
    wout1 = weight_variable([3, 3, 3, 3 * int(int(wchan) / 4), int(int(wchan) / 4)],
                            'zw10103' + str(randint(0, 100000)));
...
def isoLap():
    ...
    return L0x, L0y, L0z, tf.constant(L0x, dtype=tf.float32), tf.constant(L0y, dtype=tf.float32), tf.constant(L0z,
                                                                                                              dtype=tf.float32)
...
def normalise(x):
    ...
    div = tf.math.maximum(tf.abs(tf.contrib.distributions.percentile(x, 5.0)),
                          tf.abs(tf.contrib.distributions.percentile(x, 95.0)));
...
def norm0to1(x):
    ...
    out = tf.cond(tf.greater(tf.reduce_max(x), tf.reduce_min(x)), lambda: normalise(x), lambda: x)
    return out
...
def main(filePath, fileOutName, dataSetTrain, dataSetTest, tRand, MatOutName):
    ...
    priorUS = tf.placeholder(tf.float32, [bSize, NxUS, NzUS])
    ...
    with tf.variable_scope('transfer'):
        ...
        _, _, _, _, prior2Dconv = utun.UNETd(tf.expand_dims(priorUS, axis=3))
        NxUSconv = int(prior2Dconv.get_shape()[1])
        NzUSconv = int(prior2Dconv.get_shape()[2])
        NcUSconv = int(prior2Dconv.get_shape()[3])
        W2D3Dconv = (1) * tf.get_variable('W2D3Dconv', shape=[Nx * Ny * Nz, NxUSconv * NzUSconv * NcUSconv],
                                      initializer=tf.contrib.layers.xavier_initializer())

        prior0conv = tf.matmul(tf.tile(tf.expand_dims(W2D3Dconv, axis=0), [bSize, 1, 1]),
                            tf.reshape(prior2Dconv, [bSize, -1, 1]),
                            ) + b0
    with tf.name_scope('FCN'):
        b0fcn = bias_variable([1])
        # prior2Dconv = prior2D

        _, _, _, _, prior2Dfcn = utun.UNETd(tf.expand_dims(prior2D, axis=3))
        NxUSfcn = int(prior2Dconv.get_shape()[1])
        NzUSfcn = int(prior2Dconv.get_shape()[2])
        NcUSfcn = int(prior2Dconv.get_shape()[3])

        W2D3D = (1) * tf.get_variable('W2D3D', shape=[Nx * Ny * Nz, NxUSfcn * NzUSfcn*NcUSfcn],
                                      initializer=tf.contrib.layers.xavier_initializer())
        prior0fcn = (tf.matmul(tf.tile(tf.expand_dims(W2D3D, axis=0), [bSize, 1, 1]),
                            tf.reshape(prior2Dfcn, [bSize, -1, 1]),
                            ) + b0fcn)
    with tf.name_scope('Rest_of_net'):

        prior0 = bool_US * prior0conv + (1 - bool_US)*prior0fcn;
        ...
        if float(sys.argv[2]) != 0:  # check if we want to consider the intitial reconstruction
            inmuarecon0 = NanTo0(norm0to1(inmuarecon0))
            inmusrecon0 = NanTo0(norm0to1(inmusrecon0))
        else:
        ...
        for i_rec in range(0, Nmax):
            ...
            rel_loss_p[i_rec + 1] = 0.05 * rel_loss_p[i_rec + 1] + coeff[i_rec + 1] * (
                        0.9 * overlap(prior3D, recon_prior) +
                        0.1 * overlap(to_bin(prior3D), to_bin(recon_prior)));
    ...
    with tf.name_scope('summaries'):
        ...
        with tf.name_scope('training'):
            ...
            collection_transfer = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='transfer')
            print(collection_transfer)
            #collection_transfer = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='FCN')
            #print(collection_transfer)
            #time.sleep(60 * 2)
            train_step_transfer = tf.train.AdamOptimizer(learningRate).minimize(rel_loss, var_list=collection_transfer)
    ...
    saved_path = MatOutName[0][0:-4]+ 'best_binaryInput';
    #saver.restore(sess, saved_path);
    ''''
    vars_to_load = [i[0] for i in tf.train.list_variables(saved_path)]
    #assignment_map = {variable.op.name: variable for variable in vars_to_load}#if variable.op.name in vars_to_load}
    assignment_map = {variable.op.name: variable for variable in tf.global_variables() if
                      variable.op.name in vars_to_load}
    tf.contrib.framework.init_from_checkpoint(saved_path,assignment_map);
    '''
    ...
    for i in range(maxIter):
        ...
        if parint == 0:
            ...
            feed_train = {muarecon0: float(sys.argv[2]) * np.float32(np.array(batch['recona'])),
                          ...
                          priorUS: float(sys.argv[3]) * np.float32(np.array(batch['US'])),
                          learningRate: lVal,
                          bool_US: boolUS}
        elif parint == 1:
        ...
        if i <= 200:
            ...
            #feed_train[coeff] = np.array([0.5, 1, 0, 0]);
            _, loss_result_train = sess.run([train_step, rel_loss_p], feed_dict=feed_train);
        ...
        if (((i % 25 == 0) & (i >= val_tresh))|((i % 100 == 0) & (i < val_tresh))):  # write test
            ...
            for i_test in range(0, 1):#len(dataDbar) - 1):
                ...
                feed_test = {muarecon0: float(sys.argv[2]) * np.float32(dataDbar[i_test].test.recona[tBeg:tEnd]),
                             ...
                             priorUS: float(sys.argv[3]) * np.float32(dataDbar[i_test].test.US[tBeg:tEnd]),
                             bool_US: boolUS
                             }
                ...
                rel_loss_res_p,bbbool = sess.run([rel_loss_p, bool_US], feed_dict=feed_test)

                Ltest.append(rel_loss_res_p)

                print('iter={}, rel.lossp={}, rel.loss.tottrain={},bool={}'.format(i, rel_loss_res_p,  loss_result_train,bbbool))
            ...
            if i > val_tresh:
                ...
                for i_val in range(0, np.shape(dataDbar[1].test.recona)[0] - bSize + 1):
                    ...
                    feed_test = {muarecon0: float(sys.argv[2]) * np.float32(dataDbar[1].test.recona[tBeg:tEnd]),
                                 ...
                                 priorUS: float(sys.argv[3]) * np.float32(dataDbar[1].test.US[tBeg:tEnd]),
                                 bool_US: boolUS
                                 };
                    feed_test[coeff] = feed_train[coeff];
    ...
    for i_dataset in range(0, len(dataDbar)):
        ...
        for i_final in range(0, np.shape(dataDbar[i_dataset].test.recona)[0] - bSize + 1):
            ...
            tEnd = i_final + bSize;
            feed_test = {
                muarecon0: float(sys.argv[2]) * np.float32(dataDbar[i_dataset].test.recona[tBeg:tEnd]),
                ...
                priorUS: float(sys.argv[3]) * np.float32(dataDbar[i_dataset].test.US[tBeg:tEnd]),
                bool_US: boolUS
            };

            feed_test[coeff] = feed_train[coeff];
            test_resultp, test_STresult = sess.run([recon_prior, prior0], feed_dict=feed_test);
    ...
    # re-inizialise champion
    champion_loss_val = float(1);
    maxIter_transfer = 45000;
    boolUS = 1.0;
    for i in range(maxIter_transfer):
        ...
        if i <=1000:
            feed_train[coeff] = np.array([1, 0, 0, 0]);
            _, loss_result_train = sess.run([train_step_transfer, rel_loss_p], feed_dict=feed_train);
        elif (i < maxIter/(fact_maxiter)):
            #feed_train[coeff] = np.array([0.01,1,0,0]);
            feed_train[coeff] = np.array([0.1, 1, 0, 0]);
            _, loss_result_train = sess.run([train_step_transfer, rel_loss_p], feed_dict=feed_train);
        elif ((i >= maxIter/(fact_maxiter)) & (i < ((2*maxIter)/fact_maxiter))):
            feed_train[coeff] = np.array([0,0.5,0.5,0]);
            _, loss_result_train = sess.run([train_step_transfer, rel_loss_p], feed_dict=feed_train);
        ...
        if (((i % 25 == 0) & (i >= 100))|((i % 100 == 0) & (i < 100))): #write test
            # train_accuracy = accuracy.eval(feed_dict={imag: dataDbar.test.images[0:16], true: dataDbar.test.true[0:16]})
            # testPosit = testPos.eval(feed_dict={imag: dataDbar.test.images[0:16], true: dataDbar.test.true[0:16]})
            for i_test in range(0,1):#len(dataDbar) - 1):
                tBeg = randint(0, tRand[i_test])
                ...
                print('TRANSFER iter={}, rel.lossp={}, rel.loss.tottrain={}, bool={}'.format(i,rel_loss_res_p, loss_result_train,bbbool))
            ...
            if i > 100:
                ...
                if (mean_val_loss < champion_loss_val):
                    ...
                    print('NEW CHAMPION, SAVING ...')
                    print('Save model')
                    saved_pathT = saver.save(sess, MatOutName[0][0:-4]+ 'best_binaryInputTransfer')

    saver.restore(sess, saved_pathT);
    for i_dataset in range(0, len(dataDbar)):
        resultp = [None] * (np.shape(dataDbar[i_dataset].test.recona)[0])
        STresult = [None] * (np.shape(dataDbar[i_dataset].test.recona)[0])
        print('saving test = {}'.format(i_dataset))
        for i_final in range(0, np.shape(dataDbar[i_dataset].test.recona)[0] - bSize+1 ):
            ...
            feed_test = {muarecon0:  float(sys.argv[2]) * np.float32(dataDbar[i_dataset].test.recona[tBeg:tEnd]),
                         ...
                         prior3D: dataDbar[i_dataset].test.prior[tBeg:tEnd],
                         prior2D: float(sys.argv[3])* np.float32(dataDbar[i_dataset].test.priorslice[tBeg:tEnd]),
                         priorUS: float(sys.argv[3]) * np.float32(
                             dataDbar[i_dataset].test.US[tBeg:tEnd]),
                         bool_US: boolUS
                         };
        ...
        # ;
        print('Saving Matlab file')
        sio.savemat(MatOutName[i_dataset][0:-4]+'_Transfered.mat', dict_sio)
        print("Model saved in file: %s" % MatOutName[i_dataset][0:-4]+'_Transfered.mat')
...
specName_test = [[str(sys.argv[4])+str(sys.argv[6]),
                  ...
                  'FEMsph'+str(sys.argv[6]),
                  'FEMcomb'+str(sys.argv[6]) ]]
...
for j_proc in range(0, len(specName_train)):
    ...
    for i_logname in range(0, len(specName_test[j_proc])):
        ...
        logName[i_logname] = logNameCommon + '_train_' + specName_train[j_proc] + '_test_' + specName_test[j_proc][
            i_logname];
        savematName[i_logname] = util.default_tensorboard_dir('') + 'Matlab' + logNameCommon + '_train_' + \
                                 specName_train[j_proc] + '_test_' + specName_test[j_proc][i_logname] + '.mat';
    print(logName)
