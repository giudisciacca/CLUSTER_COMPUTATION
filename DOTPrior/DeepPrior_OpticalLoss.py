...
def ssp_getLaplacian(p, str_behaviour = 'fromSingle'):
    ...
    if str_behaviour == 'fromSingle':
        ...
        kap = ssp.diags(np.reshape(kap, [ -1])).asformat(spformat);
        Lx = np.multiply(kap, L0x);
        Ly = np.multiply(kap, L0y);
        Lz = np.multiply(kap, L0z);
...
def ssp_solve(U,s,V, data, prior_tot, alpha):
    datazeros = np.zeros([Nx * Ny * Nz * 3 * 2, ]);
    ...
    for i in range(0,np.shape(U)[0]):
        ...
        if global_behav == 'fromSingle':
            ...
            Jp = ssp_assembleJ(J, L.multiply(alpha),L.multiply(alpha), L*0, L*0);
        ...
        #solve
        out[i,:] = np.float32(ssplin.lsqr(Jp, datap)[0]);

    return np.array(out)
def get_pyLoss(U,s,V, data, prior_tot, alpha,AbsTrue, ScaTrue, PriorTrue):
    out = ssp_solve(U, s, V, data, prior_tot, alpha);
    out = np.transpose(tf.reshape(out,[bSize,2, Nz, Ny, Nx, 1]), [0,4,3,2,1, 5]);
    out_mua = 1+out[:,:,:,:,0,:];
    out_mus = np.div(1.0,3.0*(out[:,:,:,:,1,:]+1));
    #out_mus = out_mua;
    out_mua[out_mua<0] = 0
    out_mus[out_mus < 0] = 0
    rel_loss_p = np.norm(tf.subtract((prior_tot), PriorTrue)) / np.norm(PriorTrue)
    rel_loss_a = np.norm(tf.subtract((out_mua), 1+AbsTrue)) / np.norm(1+AbsTrue)
    rel_loss_s = np.norm(tf.subtract((out_mus), 1+ScaTrue)) / tf.norm(1+ScaTrue)
    return rel_loss_a + 0.1*rel_loss_s
...
def _computeGrads(op, grads):
    return [np.float32(genGrad)]

genGrad = grad(get_pyLoss)
...
def main(filePath, fileOutName, dataSetTrain, dataSetTest, tRand, MatOutName):
    ...
    with tf.name_scope('Net'):
        ...
        for i_rec in range(0,1):
            recon_prior = (UNET(muarecon0, musrecon0, tf.nn.sigmoid(prior))+ bb)

    # compute optical reconstruction and loss
    alpha = bias_variable([1])
    Vt = tf.transpose(V, [0,2,1]);
    J0 = tf.matmul(U,tf.matmul(s,Vt));
    Lprior =  getLaplacian(recon_prior, str_behaviour = 'fromSingle');
    Jp = assembleJ(J0, alpha*Lprior,alpha*Lprior, tf.zeros_like(Lprior),tf.zeros_like(Lprior))
    datap = tf.concat([ data,tf.zeros([bSize, Nx*Ny*Nz*2*3,1])],axis = 1);
    out = tf.linalg.lstsq(Jp, datap, l2_regularizer=0.0, fast=True, name=None);
    alpha = bias_variable([1])
    with ops.name_scope(None, "PySolve", [U,s,V,data,tf.nn.relu(recon_prior), alpha]) as name:
        out = py_func(func=ssp_solve,inp=[U,s,V,data,tf.nn.relu(recon_prior), alpha],Tout = tf.float32, grad = _computeGrads)
    out = tf.transpose(tf.reshape(out,[bSize,2, Nz, Ny, Nx, 1]), [0,4,3,2,1, 5]);
    out_mua = 1+out[:,:,:,:,0,:];
    out_mus = tf.div(1.0,3.0*(out[:,:,:,:,1,:]+1));
    #out_mus = out_mua;

    with tf.name_scope('optimizer'):
        rel_loss_p = tf.norm(tf.subtract((recon_prior), prior3D)) / tf.norm(prior3D)
        rel_loss_a = tf.norm(tf.subtract(tf.nn.relu(out_mua), 1+truea)) / tf.norm(1+truea)
        rel_loss_s = tf.norm(tf.subtract(tf.nn.relu(out_mus), 1+trues)) / tf.norm(1+trues)
        rel_loss = rel_loss_a + 0.1*rel_loss_s#+0.4*rel_loss_p;
    ...
    for i in range(maxIter):
        ...
        if i % 50 == 0: #write test
            ...
            for i_test in range(0,len(dataDbar)):
                ...
                rel_loss_res_p,rel_loss_res_a,rel_loss_res_s, rel_loss_res= sess.run([ rel_loss_p,rel_loss_a,rel_loss_s, rel_loss],
                                                                            feed_dict=feed_test)

                Ltest.append(rel_loss_res_p)

                print('iter={}, rel.lossp={}, rel.lossa.test={}, rel.losss.test={}, rel.loss.tottrain={}'.format(i,rel_loss_res_p,rel_loss_res_a,rel_loss_res_s, rel_loss_res))
    # SAVING
