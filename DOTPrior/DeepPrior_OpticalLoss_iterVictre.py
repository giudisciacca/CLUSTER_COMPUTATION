...
def overlap(true, guess):
    weight = tf.reshape(tf.ones_like(true),[-1]);
    true = tf.multiply(weight,tf.reshape(tf.nn.relu(true), [-1]));
    guess = tf.multiply(weight,tf.reshape(tf.nn.relu(guess), [-1]));
...
def main(filePath, fileOutName, dataSetTrain, dataSetTest, tRand, MatOutName):
    ...
    with tf.name_scope('Net'):
        ...
        #prior2Dconv = prior2D
        W2D3D = (1 )*tf.get_variable('W2D3D',shape=[Nx*Ny*Nz*chan,NxUSconv*NzUSconv*NcUSconv], initializer=tf.contrib.layers.xavier_initializer())
        prior0 =(tf.matmul(tf.tile(tf.expand_dims( W2D3D, axis = 0),[bSize, 1,1]),
                                      tf.reshape(prior2Dconv, [bSize,-1,1]),
                                    )+b0)
        #recon_prior = tf.transpose(tf.reshape(prior0 , [bSize, Nx,Nz,Ny,chan]), [0,1,3,2,4])
        recon_prior = tf.transpose(tf.reshape(prior0, [bSize,Nz, Ny, Nx, chan]), [0, 3, 2, 1, 4])
        recon_prior = NanTo0(recon_prior);
        Nmax = int(sys.argv[5])
        rel_loss_p = [None]*(Nmax+1)
        rel_loss_p[0] = coeff[0] * (tf.norm(tf.subtract(recon_prior, tf.tile(prior3D, [1,1,1,1,chan]))) / (tf.norm(0.0+tf.tile(prior3D, [1,1,1,1,chan]))))
        rel_loss_p[0] = 0.05*rel_loss_p[0] + coeff[0] * (0.9* overlap(tf.tile(prior3D, [1,1,1,1,chan]), recon_prior) +
                                                         0.1* overlap(to_bin(tf.tile(prior3D, [1,1,1,1,chan])), to_bin(recon_prior)));
...
train_num = [ np.multiply( [ 124 - bSize -1, 124 - bSize -1,72-bSize -1,100,100],1)];
