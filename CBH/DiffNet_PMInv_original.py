...
#   initial = tf.truncated_normal(shape, stddev=0.005)
    return 0.1*tf.get_variable(layernum, shape = shape, initializer=tf.contrib.layers.xavier_initializer(uniform = False))
...
#        kappaEst=tf.contrib.layers.conv2d(kappaEst,5,3,activation_fn=None)
    ...
    x_update = x_update + tf.multiply(dt , (xUp + xDown + xLeft + xRight - xDiag + xUpRight + xDownLeft - xUpLeft - xDownRight) ) #Test +xDiag
        ...
        if i % 20 == 0:
            ...
            for i_test in range(0,len(dataDbar)):
                ...
                feed_test = {imag: dataDbar[i_test].test.images[tBeg:tEnd],
                             true: dataDbar[i_test].test.true[tBeg:tEnd], learningRate:lVal}

                test_accuracy, test_result = sess.run([loss, y_diff],
                                                      feed_dict=feed_test)

                loss_result, rel_loss_res, merged_summary_result,lV = sess.run([loss, rel_loss, merged_summary,learningRate],
                                                                            feed_dict=feed_test)
...
for j_proc in range(0, len(specName_train)):
    ...
    for i_logname in range(0, len(specName_test[j_proc])):
        ...
        logName[i_logname] = logNameCommon + '_train_' + specName_train[j_proc] + '_test_' + specName_test[j_proc][
            i_logname];
        savematName[i_logname] = util.default_tensorboard_dir(name) +'Matlabnormtrue' + '_train_' + specName_train[j_proc] + '_test_' + specName_test[j_proc][i_logname] +'_lval_'+str(np.single(sys.argv[2]))+  '.mat';
    print(logName)
