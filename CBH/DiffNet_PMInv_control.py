...
def assembleXupdate2D_old(x_update, kappa, dt, order):
    x_out = tf.zeros_like(x_update);
    count_k = 0;
    for i in range(-1, 2):
        for j in range(-1, 2):
...
def main(filePath, fileOutName, dataSetTrain, dataSetTest, tRand, MatOutName):
    ...
    for i in range(maxIter):
        ...
        #      test1 = batch[0]
        #      test2 = batch[1]
        feed_train = {imag: batch[0], true: batch[1], contr: batch[2], learningRate: lVal}
...
for j_proc in range(0, len(specName_train)):
    ...
    for i_logname in range(0, len(specName_test[j_proc])):
        ...
        savematName[i_logname] = util.default_tensorboard_dir(name) + 'Matlab' + '_train_' + specName_train[
            j_proc] + '_test_' + specName_test[j_proc][i_logname] +'_lval_'+str(np.single(sys.argv[2]))+ '.mat';
    print(logName)
