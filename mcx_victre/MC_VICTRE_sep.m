function [outdata,taxis] = MC_VICTRE(strlab, mua, musp,srcpos,detpos)
...
detposs = [(offset+[0,0,2])+((1/(u_mm))*detpos),2*ones(size(detpos,1),1)];
...
cfg.nphoton = 10^9;
cfg.maxdetphoton = 4*10^6;

cfg.tstart =0;% (i_t-1)* tstep ;
cfg.tstep = tstep;
cfg.tend = tstep*(nstep-1);
dets = 1:8;
srcs = 1:8;
    ...
    for i_src = srcs
        for i_det =dets
            if ~isempty(outdet(srcs==i_src).data)
            	...
            	tmp.ppath = u_mm* tmp.ppath(tmp.detid==i_det,:);
