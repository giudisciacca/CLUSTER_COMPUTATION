function [outdata,taxis] = MC_VICTRE2(strlab, mua, musp,srcpos,detpos)
...
while flag_coll == 0
    ...
    %mcxpreview(cfg)
    evalc('[fluence,outdet] = mcxlab(cfgs);');%,'preview');
    %[~,outdet] = mcxlab(cfgs(:));
    %fluence_surf(:,:,:) = squeeze(squeeze(fluence.data(:,:,2,:)));
    for i_src = srcs
        for i_det =dets
            tmp = fluence(i_src);
            counts=extract_data(tmp.data,cfgs(i_src).detpos(i_det,:));
            if sum(dat(:,i_det,i_src)) < tol_ph
                dat(:,i_det, i_src)= dat(:,i_det, i_src)+counts; 
            end
        end
        n_coll =  sum(squeeze(dat(:,:,i_src)),1);
        if min(n_coll(:)) >= tol_ph
            srcs(srcs == i_src)=[];        
        end
    end
    disp(['lowest photon count:',num2str( min(min(sum(squeeze(dat),1))))])
...
function tpsf = extract_data(data,detpos)
    [xx,yy,zz] = ndgrid(1:size(data,1),1:size(data,2),1:size(data,3));
    %tpsf = zeros(size(data,4),1);
    pos = zz==detpos(3) .* ((xx-detpos(1)).^2 + (yy-detpos(2)).^2 <=detpos(4));
    
    [ix,iy,iz]=ind2sub(size(xx), find(pos==1));    
    data = data(ix,iy,iz,:);
    tpsf = squeeze(sum(sum(sum(data,1),2),3))*25*10^(-12)%*0.005^(3)*25;
    return
end
