...
ilambda=lam;
%calc
backup =['tmp_backup_',num2str(N),'_l',num2str(lam),'_q',num2str(q)];

if q == 1
[hete(:,:,:),~ ]= MC_VICTRE(structureMass,structopt.mua(:,:,:,ilambda),structopt.musp(:,:,:,ilambda), DOT.Source.Pos ,DOT.Detector.Pos, backup);
homo = 0*hete;
elseif q == 2
[homo(:,:,:),taxis] = MC_VICTRE(structure,structopt.mua0(:,:,:,ilambda),structopt.musp0(:,:,:,ilambda), DOT.Source.Pos ,DOT.Detector.Pos, backup);
hete = homo*0;
else 
disp('what now')
end
...
% save theodata
save(['Test_Standard_MCtheo_',num2str(N),'_l',num2str(lam),'_q',num2str(q)],'hete','homo')
...
% save forwdata
save(['Test_Standard_MC_',num2str(N),'_l',num2str(lam),'_q',num2str(q)],'RefTD','DataTD')
