...
for count_liquido_scatter=1:numel(scattering_rota)
    ...
    srcpos = [1, 1, 10];
    N_src = size(srcpos,1);
    cfg.srctype= 'pattern'; %'gaussian';%
    cfg.srcparam1 = [DIM(1),0,0,DIM(1)];%;[3 0 0 0]; % 
    ...
    prov_MNIST(DIM(1)/2 - 13: DIM(1)/2 + 14,DIM(2)/2 - 13: DIM(2)/2 + 14, :) = MNIST;
    MNIST = prov_MNIST;
    rng('default');fixed_rand = randi(20, mnist_size, 2) - 10;
    for i_tr = 1:mnist_size
        variation = generateRandomKappa(1, 1, 1);
        mnist_layer_abs(:,:,i_tr) = circshift(MNIST(:,:,i_tr) ,fixed_rand(i_tr,:));
        MNIST(:,:,i_tr) = circshift(MNIST(:,:,i_tr) ,fixed_rand(i_tr,:)) ./ (variation/ max(variation(:)));
    end
    %MNIST(:,:,1) = 0;
...
%             end
    ...
    disp('saving')
    dirname = sprintf('/scratch0/NOT_BACKED_UP/gdisciac/mcx_files/DOCM_%s_ph%g_radius_%g_dist2scalp%g_liqMus%g/', midname, cfg.nphoton, RAD_IN, SKULL2BRAIN,scattering_rota(count_liquido_scatter));
