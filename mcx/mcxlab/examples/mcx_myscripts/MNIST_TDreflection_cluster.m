...
for count_liquido_scatter=1:numel(scattering_rota)
    ...
    cfg.isspecular = 0;
    cfg.unitinmm = 1;
    [srcx,srcy,srcz] = meshgrid([10,15,20,25,30], [10,15,20,25,30], 0);
    srcpos = [srcx(:), srcy(:), srcz(:)];
    N_src = size(srcpos,1);
    cfg.srctype='gaussian';
    cfg.srcparam1 = [2,0,0,0];
    cfg.srcdir=[0 0 1];
% % %     srcpos = [1, 1, 0];
% % %     N_src = size(srcpos,1);
...
% % %     cfg.srcdir=[0 0 1];
    ...
    rng('default');fixed_rand = randi(6, mnist_size, 2) - 3;
    ...
    if RUN     
        ...
        whole_homCW = zeros(DIM(1),DIM(2), numel(cfg.tstart:cfg.tstep:cfg.tend)-1, N_src, mnist_size, 'single');
        whole_mnistCW = zeros(DIM(1),DIM(2), numel(cfg.tstart:cfg.tstep:cfg.tend)-1,N_src, mnist_size, 'single'); 
        tic;
        for i_mnist = 1:mnist_size
            ...
            for i_src = 1:N_src
                ...
                disp(srcpos(i_src,:));
                 ...
                 whole_homCW0(:,:,:,:) = flux_hom_data(:,:,:,:); 
                 hom_layer_photons(:,:,i_src, i_mnist) = simple_project_sph2pl(idx_sphere_in, sum(whole_homCW0,4), SKULL2BRAIN, RAD_IN);
                 whole_homCW(:,:,:,i_src, i_mnist) = single( squeeze(flux_hom_data(:,:,1,:)));    
                 ...
                 whole_mnistCW0(:,:,:,:) = flux_mnist_data(:,:,:,:); 
                 mnist_layer_photons(:,:,i_src, i_mnist) = simple_project_sph2pl(idx_sphere_in, sum(whole_mnistCW0,4), SKULL2BRAIN, RAD_IN);
                 whole_mnistCW(:,:,:,i_src, i_mnist) = single( squeeze(flux_mnist_data(:,:,1,:)));    
             end

            whole_homCW = whole_homCW(:,:,:, :,:);
            whole_mnistCW = whole_mnistCW(:,:,:, :,:);
